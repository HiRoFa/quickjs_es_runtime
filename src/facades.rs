...
thread_local! {
    ...
    static REALM_ID_LRU_CACHE: RefCell<LruCache<String, ()>> = RefCell::new(LruCache::new(NonZeroUsize::new(128).unwrap()));
}
        ...
        Right(consumer) => {
            ...
            REALM_ID_LRU_CACHE.with(|cache_cell| {
                ...
                if cache.len() == cache.cap().get() {
                    if let Some((_evicted_key, _evicted_value)) = cache.pop_lru() {
                        // cleanup evicted key
                        //QuickJsRuntimeAdapter::remove_context(evicted_key.as_str())
                        //    .expect("could not destroy realm");
                    }
                }
