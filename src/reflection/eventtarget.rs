...
use crate::quickjs_utils::{functions, maps, parse_args, primitives};
...
pub(crate) fn impl_event_target(proxy: Proxy) -> Proxy {
    ...
    if proxy.is_event_target {
        proxy = proxy
            .native_method("addEventListener", Some(add_event_listener))
            .native_method("removeEventListener", Some(remove_event_listener))
            .native_method("dispatchEvent", Some(dispatch_event));
    }
    if proxy.is_static_event_target {
        proxy = proxy
            .static_native_method("addEventListener", Some(add_event_listener))
            .static_native_method("removeEventListener", Some(remove_event_listener))
            .static_native_method("dispatchEvent", Some(dispatch_event));
    }
...
unsafe extern "C" fn add_event_listener(
    ...
    let res = QuickJsContext::with_context(ctx, |q_ctx| {
        ...
        if args.len() < 2 || !args[0].is_string() || !functions::is_function_q(q_ctx, &args[1]) {
            Err(EsError::new_str("add_event_listener requires at least 2 arguments (eventId: String and Listener: Function"))
        } else {
...
unsafe extern "C" fn remove_event_listener(
    ...
    QuickJsContext::with_context(ctx, |_q_ctx| {
        let _args = parse_args(ctx, argc, argv);

        let _this_ref = JSValueRef::new(ctx, this_val, true, true, "remove_event_listener_this");
...
unsafe extern "C" fn dispatch_event(
    ...
    QuickJsContext::with_context(ctx, |_q_ctx| {
        ...
        let _this_ref = JSValueRef::new(ctx, this_val, true, true, "dispatch_event_this");
